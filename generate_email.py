...
def get_arxiv_paper(query: str, debug: bool = False) -> list[ArxivPaper]:
    ...
    if "Feed error for query" in feed.feed.title:  # type: ignore
        raise Exception(f"Invalid ARXIV_QUERY: {query}.")

    papers = []
    all_paper_ids = []
    for index, paper in enumerate(feed.entries):
        if paper.arxiv_announce_type == "new":
            all_paper_ids.append(paper.id.removeprefix("oai:arXiv.org:"))  # type: ignore
        else:
            logger.warning(
                f"Skipping paper {index + 1}: {paper.title} for not being new."
            )
...
if __name__ == "__main__":
    ...
    logger.info("Retrieving Arxiv papers for {}...", args.arxiv_query)
